...
import { REGION, KMA_GRIDS } from '../src/data/busanRegion.js'
...
async function fetchFireRisk() {
  ...
  const gg = items.filter((x) => x.doname === REGION.sido)
  if (gg.length === 0) throw new Error('부산 데이터 없음')
...
// 기상청 단기예보 격자(nx, ny)는 busanRegion.js에서 import (구청·군청 좌표 → LCC 변환 자동 산출).
...
async function fetchKmaForGrid(nx, ny, base, attempt = 1) {
  ...
  if (res.status === 429 && attempt < 7) {
    await sleep(3000 * attempt) // 3s, 6s, 9s, 12s, 15s, 18s
...
async function fetchWeather() {
  ...
  for (const [city, grid] of Object.entries(KMA_GRIDS)) {
    ...
    await sleep(1200) // throttle ~0.8 req/s (429 회피)
  }

  if (okCount === 0) throw new Error(`전 시군 실패: ${failCount}개`)

  return {
    _summary: `${okCount}/${Object.keys(KMA_GRIDS).length}개 구·군 (${base.base_date} ${base.base_time} 발표 기준)`,
    fetched_at: new Date().toISOString(),
...
// ────────────────────────────────────────────────────────────────
// 3. 농진청 토양도 V2 - 부산 구·군별 대표 PNU로 토양 단면 조회
//    응답: XML 전용 (Deepsoil_Qlt_Cd / Deepsoil_Ston_Cd / Soilslope_Cd)
// ────────────────────────────────────────────────────────────────
import { REPRESENTATIVE_PNU, FALLBACK_BJDONG } from '../src/data/busanPnu.js'
...
async function fetchSoilForCity(city, baseEntry) {
  ...
  // 1차: 시청 인근 산지(2) 우선, 본번 1·10·50·100·500·1000 다변화
  const candidates = [
    `${bjdong}2` + '0001' + '0000',
    `${bjdong}2` + '0010' + '0000',
    `${bjdong}2` + '0050' + '0000',
  ...
  // 2차: 도심 시군이면 fallback 산림 법정동 추가 (산지 2 + 본번 1·10·100)
  const fallbacks = FALLBACK_BJDONG[city] || []
...
async function fetchSoil() {
  ...
  return {
    _summary: `${okCount}/${Object.keys(REPRESENTATIVE_PNU).length}개 구·군 (대표 PNU 샘플링)`,
    fetched_at: new Date().toISOString(),
