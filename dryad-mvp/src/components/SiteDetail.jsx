...
export default function SiteDetail({ site, records, complaints = [], cityComplaints = [], onAddRecord }) {
  ...
  return (
    <div className="scrollbar-thin flex h-full flex-col overflow-y-auto">
      ...
      <Section title="기본정보">
        <InfoGrid
          rows={[
            ...
            ...(matureTreePotential(site)
              ? [[
                  '성목 1주 잠재(참고)',
                  `${matureTreePotential(site).perTreeKgc} kgC ≈ ${matureTreePotential(site).perTreeTco2} tCO₂/주·년 (라이다 실측)`
                ]]
              : []),
            [
              '최근 관리 후',
      ...
      <Section title="AI 추천 조치" hint="규칙 기반 추론 + Claude 자연어 진단 · 행정 판단 보조용">
        ...
        <AiDiagnosis site={site} rec={rec} complaints={complaints} />
    ...
    <span className="flex items-center gap-1">
      ...
      {approx && (
        <span
          ...
          title="원본에 정확한 좌표가 없어 구·군 중심 기준 근사 위치입니다"
        >
          좌표근사
        </span>
      )}
...
// 사이트별 추가 메타데이터 표시 (부산/전국 환경데이터·SDR·도시숲표준지·VWorld PNU·집계 정보)
function ExtendedDataSection({ site }) {
  const items = []
  // 산림빅데이터거래소 — 산림병해충 발생현황
  if (site._pest_type) {
    items.push({
      label: '병해충 유형',
      value: site._pest_type,
      source: '산림빅데이터거래소',
      hint: '식생·피해이력 점수에 반영',
      danger: true
    })
  }
  if (site._pest_damage_count != null) {
    items.push({
      label: '피해(고사)목 수',
      value: site._pest_damage_count.toLocaleString() + '본',
      source: '산림빅데이터거래소',
      danger: true
    })
  }
  ...
  if (site._tree_canopy_cvg != null) {
    items.push({
      label: '수관 폐쇄율',
      value: (site._tree_canopy_cvg * 100).toFixed(1) + '%',
      source: '부산/전국 환경데이터',
      hint: '0% = 완전 노출 · 100% = 완전 폐쇄. 식생 점수에 가중'
    })
  }
  if (site._tree_canopy_avg_hgt != null) {
    items.push({
      label: '평균 수목 높이',
      value: site._tree_canopy_avg_hgt.toFixed(1) + ' m',
      source: '부산/전국 환경데이터'
    })
  }
  if (site._forest_carbon_uptake != null) {
    items.push({
      label: '산림 탄소흡수량',
      value: site._forest_carbon_uptake.toFixed(3) + ' t CO₂/yr',
      source: '부산/전국 환경데이터',
      hint: 'ESG·NDC 정량화 (격자 단위)'
    })
  }
  if (site._emd_park_score != null) {
    items.push({
      label: '읍면동 공원 종합점수',
      value: site._emd_park_score.toFixed(1) + ' / 100',
      source: '부산/전국 환경데이터',
      hint: '공원 평가 (관리공백 가산에 반영)'
    })
  }
  if (site._flood_risk_idx != null) {
    items.push({
      label: '구·군 홍수위험 지수',
      value: site._flood_risk_idx.toFixed(3) + ' / 1.0',
      source: '부산/전국 환경데이터',
      hint: '피해이력 점수에 40% 가중'
    })
  }
  ...
  if (site._sdr_density != null) {
    items.push({
      ...
      value:
        ...
        ' ' +
        (site._sdr_unit || '본/m') +
        (site._sdr_adjustment > 0 ? ` (식생 +${site._sdr_adjustment})` : site._sdr_adjustment < 0 ? ` (${site._sdr_adjustment})` : ''),
      source: 'ETL 계산',
      hint: '본수/길이 또는 본수/면적 — 과밀 시 식생 취약성 ↑',
      danger: site._sdr_adjustment > 0
    })
  }
  // 도시숲표준지 (파주)
  if (site._vitality_grade != null) {
    items.push({
      label: '수목 활력도 등급',
      value: site._vitality_grade + ' / 5',
      source: '도시숲표준지',
      hint: '1=최상 · 5=고사 직전. 식생 점수에 직접 반영'
    })
  }
  if (site._biodiversity_grade != null) {
    items.push({
      label: '종다양성 등급',
      value: site._biodiversity_grade + ' / 5',
      source: '도시숲표준지'
    })
  }
  ...
  if (site._tree_count != null) {
    items.push({
      label: '집계 본수',
      value: site._tree_count.toLocaleString() + '그루',
  ...
  if (site._species_mix) {
    items.push({
      label: '수종 분포 (상위 3)',
      value: site._species_mix,
      source: site.source_dataset
    })
  }
  if (items.length === 0) return null
  return (
    <Section title="추가 분석 데이터" hint="부산/전국 환경데이터·도시숲표준지·VWorld·집계 메타">
      <ul className="space-y-1.5">
        {items.map((it, idx) => (
          <li
            key={idx}
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <span className={`tabular-nums font-semibold ${it.danger ? 'text-red-700' : 'text-slate-900'}`}>
                {it.value}
              </span>
            </div>
            <div className="mt-0.5 flex items-center justify-between gap-2 text-[10px] text-slate-400">
              <span>{it.hint || ''}</span>
              <span className="rounded-sm bg-slate-100 px-1 py-px font-medium text-slate-500">
                {it.source}
              </span>
            </div>
          </li>
