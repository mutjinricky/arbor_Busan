...
const SCHEMA_LABEL = {
  ...
  street_tree: '가로수길 표준',
  park: '도시공원 표준',
  pest_outbreak: '산림병해충 발생현황 (산림빅데이터거래소)',
  unknown: '인식 실패'
}
        ...
        <div className="space-y-3 px-5 py-4">
          <label className="block cursor-pointer rounded-lg border-2 border-dashed border-slate-300 px-4 py-8 text-center hover:border-forest-400 hover:bg-forest-50/30">
            ...
            <div className="mt-1 text-[11px] text-slate-500">
              data.go.kr · 경기 데이터 포털에서 받은 원본 CSV를 그대로 올리세요
            </div>
          </label>
          ...
          <div className="rounded-md border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] leading-relaxed text-slate-500">
            지원 표준데이터: <br />
            ① 산림청 100대 소나무숲 (시군/읍면/리/번지/대상면적/주요명칭)<br />
            ② 경기도 가로수길정보표준데이터 (가로수길명/시군구/위도/경도 등)<br />
            ③ 전국도시공원정보표준데이터 (공원명/소재지/공원유형 등)<br />
            ④ 산림빅데이터거래소 산림병해충 발생현황 (재선충/병해충/고사목 + 시군·위경도)
          </div>
          ...
          <div className="rounded-md border border-forest-200 bg-forest-50/40 px-3 py-2 text-[11px] text-slate-700">
            ...
            <div className="flex flex-col gap-1">
              <div>
                <a
                  href="/samples/demo_streets_gyeonggi.csv"
                  download
                  className="inline-flex items-center gap-1 text-forest-700 hover:underline"
                >
                  📥 demo_streets_gyeonggi.csv
                </a>
                <span className="ml-1.5 text-slate-500">
                  — 경기 12개 가로수길 (수원 화성행궁로·일산 호수공원 등)
                </span>
              </div>
              <div>
                <a
                  href="/samples/demo_pineforest_gyeonggi.csv"
                  download
                  className="inline-flex items-center gap-1 text-red-700 hover:underline"
                >
                  📥 demo_pineforest_gyeonggi.csv
                </a>
                <span className="ml-1.5 text-slate-500">
                  — 경기 16개 소나무숲 (가평·포천 잣나무·소나무 고위험)
                </span>
              </div>
              <div>
                <a
                  href="/samples/demo_highrisk_streets_gyeonggi.csv"
                  download
                  className="inline-flex items-center gap-1 text-red-700 hover:underline"
                >
                  📥 demo_highrisk_streets_gyeonggi.csv
                </a>
                <span className="ml-1.5 text-slate-500">
                  — 경기 8개 고위험 가로수 (재선충·산불 위험 구간)
                </span>
              </div>
            </div>
          </div>
