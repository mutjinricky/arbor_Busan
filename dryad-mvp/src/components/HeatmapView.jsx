    ...
    <div className="scrollbar-thin flex h-full flex-col overflow-auto bg-white px-5 py-4">
      <div className="mb-3">
        ...
        <div className="mt-0.5 text-[11px] text-slate-500">
          부산 {matrix.length}개 구·군 · 총 {matrix.reduce((a, c) => a + c.count, 0)}개 사이트 ·
          각 셀은 구·군 내 평균 점수. 클릭 시 해당 구·군으로 필터링됩니다.
        </div>
      </div>
      ...
      <table className="text-xs">
        <thead className="sticky top-0 z-10 bg-white shadow-[0_1px_0_0_#e2e8f0]">
          <tr className="border-b border-slate-200">
            <th className="sticky left-0 z-20 bg-white px-2 py-2 text-left font-medium text-slate-500">
              구·군
            </th>
        ...
        <tbody>
          {matrix.map((row) => {
            ...
            return (
              ...
              >
                <td className="sticky left-0 whitespace-nowrap bg-inherit px-2 py-0.5 font-semibold text-slate-800">
                  {row.city}
                </td>
                <td className="px-2 py-0.5 text-right tabular-nums text-slate-600">
                  {row.count}
                </td>
                {FACTOR_KEYS.map((k) => {
                  ...
                  return (
                    <td key={k} className="px-1 py-0.5">
                      <div
                        className="mx-auto flex h-6 w-11 items-center justify-center rounded-md text-[11px] font-bold tabular-nums"
                ...
                <td className="px-1 py-0.5">
                  <div
                    className="mx-auto flex h-7 w-12 items-center justify-center rounded-md text-[12px] font-bold tabular-nums"
                    style={(() => {
