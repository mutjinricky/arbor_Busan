// 브라우저용 CSV 파서.
// 부산 공공데이터 표준 CSV (가로수/도시공원/소나무숲)를 받아 SITE 객체 배열로 변환한다.
// 인코딩은 UTF-8 또는 EUC-KR 자동 감지. EUC-KR은 TextDecoder('euc-kr')로 처리.
//
// 지역 설정(구·군 중심좌표·기관코드·정규화)은 busanRegion.js에서 import.
...
function detectSchema(headers) {
  ...
  // 산림빅데이터거래소 — 산림병해충(재선충 등) 발생현황: 병해충/피해/고사목 + 위치 컬럼
  if (
    (has('재선충') || has('병해충') || has('고사목') || has('피해목') || has('소나무재선충')) &&
    (has('시군') || has('시도') || has('지역') || has('위도') || has('경도') || has('주소'))
  ) {
  ...
  const base =
    {
      ...
      // 병해충 발생지: 식생 취약성·피해이력이 본질적으로 높음
      pest_zone: { fire: 55, weather: 55, vegetation: 80, soil: 45, gap: 60, damage: 80 }
...
function parsePineForest(row, idx) {
  ...
  return {
    id: `${siteIdPrefix(city)}-PINE-UP-${idx}`,
...
function parseStreetTree(row, idx) {
  ...
  return {
    id: `${siteIdPrefix(city)}-STREET-UP-${idx}`,
    name,
    type: 'street_tree',
    city,
...
function parsePark(row, idx) {
  ...
  return {
    id: `${siteIdPrefix(city)}-PARK-UP-${idx}`,
...
// 산림빅데이터거래소 — 산림병해충(소나무재선충 등) 발생현황 CSV.
// 발생 지점을 병해충 발생지 사이트로 변환 (식생·피해이력 위험 기본 상향).
function parsePestOutbreak(row, idx) {
  const cityRaw = pickField(row, ['시군구', '시군', '시군구명', '지역', '발생지역', '시도'])
  let city = normalizeCity(cityRaw) || normalizeCity(pickField(row, ['주소', '소재지', '발생지']))
  let lat = Number(pickField(row, ['위도', 'Y좌표', 'lat', '위도(도)']))
  let lng = Number(pickField(row, ['경도', 'X좌표', 'lng', 'lon', '경도(도)']))
  let approx = false
  if (!isFinite(lat) || !isFinite(lng) || lat === 0 || lng === 0) {
    if (!city) return null
    const c = CITY_CENTROIDS[city]
    const seedKey = `pest-${city}-${idx}`
    lat = c[0] + jitter(seedKey + 'lat')
    lng = c[1] + jitter(seedKey + 'lng')
    approx = true
  }
  // 좌표만 있고 구·군 미상이면 부산 범위 밖일 수 있으나 일단 표시 (시군은 추후 역지오코딩)
  if (!city) city = '기타'
  const pest = pickField(row, ['병해충명', '병해충', '피해유형', '구분']) || '소나무재선충병'
  const species = pickField(row, ['수종', '주요수종', '피해수종']) || '소나무'
  const count = pickField(row, ['고사목', '피해목', '피해본수', '고사목본수', '본수'])
  const namePlace = pickField(row, ['발생지', '명칭', '소재지', '주소']) || city
  return {
    id: `PEST-UP-${idx}`,
    name: `${pest} 발생지 (${namePlace})${count ? ` · 피해 ${count}본` : ''}`,
    type: 'pest_zone',
    city,
    address: pickField(row, ['주소', '소재지', '발생지']) || (city !== '기타' ? `${REGION.sido} ${city}` : ''),
    latitude: round(lat, 6),
    longitude: round(lng, 6),
    area_m2: null,
    length_m: null,
    main_species: species,
    managing_agency: city !== '기타' ? `${city} 산림녹지과` : '산림청',
    source_dataset: '산림빅데이터거래소 (산림병해충 발생현황)',
    source_url: 'https://www.bigdata-forest.kr',
    _coord_approx: approx,
    _pest_type: pest,
    _pest_damage_count: count ? Number(String(count).replace(/[^0-9.]/g, '')) || null : null
  }
}
