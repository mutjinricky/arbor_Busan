...
import { useSites, INITIAL_RECORDS } from './data/loadSites.js'
import { enrichSite, applyComplaintBonus } from './lib/risk.js'
import { recommendActions } from './lib/recommend.js'

export default function App() {
  const { sites: SITES, realCount: REAL_COUNT, sampleCount: SAMPLE_COUNT, loading: sitesLoading } = useSites()
  ...
  // 사이트 로딩 완료 시 최고 위험도 사이트 자동 선택 (사용자가 아직 클릭 안 했을 때만)
  useEffect(() => {
    if (selectedId || sitesLoading || enrichedAll.length === 0) return
    const top = [...enrichedAll].sort((a, b) => b.total_risk_score - a.total_risk_score)[0]
    if (top) setSelectedId(top.id)
  }, [selectedId, sitesLoading, enrichedAll])
        ...
        <aside className="col-span-4 flex flex-col overflow-hidden border-r border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-4 py-3">
            <div className="text-xs font-semibold text-slate-700">
              산림 수목관리 통합 현황 <span className="font-normal text-slate-400">· 부산 16개 구·군 1차 파일럿</span>
            </div>
            <div className="text-[10px] text-slate-400">
              산림 공공데이터 ETL 산출 + 실시간 API 연동 (전국 확산 대상)
            </div>
          </div>
